<?php

// translations for luttje/filament-user-attributes
return [
    'add_attribute' => 'Voeg attribuut toe',
    'checkbox_display_no' => 'Nee',
    'checkbox_display_yes' => 'Ja',
    'toggle_display_no' => 'Nee',
    'toggle_display_yes' => 'Ja',
    'common' => 'Algemeen',
    'customizations_for' => 'Aanpassingen voor :type',
    'manage_user_attributes' => 'Beheer attributen',
    'name_already_exists' => 'Deze naam bestaat al, attributen kunnen niet dezelfde naam hebben.',
    'order_sibling_help' => 'Kies het bestaande attribuut om deze ervoor of erna te ordenen.',
    'ordering_form' => 'In Formulier',
    'ordering_table' => 'In Tabel',
    'ordering' => 'Volgordelijkheid',
    'select_sibling' => 'Selecteer naastgelegen attribuut',
    'suffix_page' => ' Pagina',
    'suggestions_help' => "Door komma's gescheiden lijst van suggesties voor de gebruiker om uit te kiezen.",

    /**
     * Names of the attributes that occur in fields.
     */
    'attributes' => [
        'config' => 'configuratie',
        'decimal_places' => 'decimale plaatsen',
        'default' => 'standaard',
        'format' => 'formaat',
        'label' => 'label',
        'maximum' => 'maximum',
        'minimum' => 'minimum',
        'name' => 'naam',
        'order_position_after' => 'na',
        'order_position_before' => 'voor',
        'order_position_hidden' => 'verborgen',
        'order_position' => 'positie t.o.v. naastgelegen attribuut',
        'order_sibling_at_end' => 'Aan het einde',
        'order_sibling' => 'naastgelegen attribuut',
        'placeholder' => 'placeholder',
        'required' => 'vereist',
        'resource_type' => 'bron type',
        'suggestions' => 'suggesties',
        'type' => 'type',
    ],
];
