...
it('adds a trait to a class that does not already use it', function () {
    ...
    $modifiedCode = \Luttje\FilamentUserAttributes\CodeGeneration\CodeModifier::addTrait($code, $traitName);
...
it('adds an interface to a class that does not implement it', function () {
    ...
    $modifiedCode = \Luttje\FilamentUserAttributes\CodeGeneration\CodeModifier::addInterface($code, $interfaceName);
...
it('correctly prefixes class names with a backslash if not already present', function () {
    $className = 'TestClass';
    $fullyQualified = \Luttje\FilamentUserAttributes\CodeGeneration\CodeModifier::fullyQualifyClass($className);
